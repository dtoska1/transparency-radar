...
const DETAIL_URL_PATH_PATTERN = /^\/[^/]+\/$/;
const ENTRY_CONTENT_SELECTOR = '.entry-inner';
...
export function isOfficialVloreKonsultimeDetailUrl(value: string): boolean {
  try {
    const url = new URL(value);
    return url.hostname === SOURCE_ORIGIN && DETAIL_URL_PATH_PATTERN.test(url.pathname);
  } catch {
    return false;
  }
}

export function extractVloreKonsultimeDocumentLinks(
  html: string,
  detailUrl: string,
): DetailDocumentLink[] {
  ...
  $(`${ENTRY_CONTENT_SELECTOR} a[href*="/wp-content/uploads/"]`).each((_i, el) => {
    ...
    if (url.hostname !== SOURCE_ORIGIN) return;
...
export class VloreKonsultimeDocumentEnricher extends BaseScraper {
  ...
  async runWithStats(): Promise<EnrichmentStats> {
    assertDevDatabase(undefined, 'Vlorë konsultime document enrichment');
    ...
    this.logger.info(totals, 'Vlorë konsultime document enrichment complete');
    return totals;
  }
  ...
  async enrichKonsultim(row: KonsultimDocumentRow): Promise<EnrichmentStats> {
    ...
    if (!isOfficialVloreKonsultimeDetailUrl(row.sourceUrl)) {
      stats.rowsSkipped += 1;
      this.logger.warn(
    ...
    const links = extractVloreKonsultimeDocumentLinks(html, row.sourceUrl);
    if (links.length === 0) {
      this.logger.info(
        { konsultimId: row.id, sourceUrl: row.sourceUrl },
  ...
  async enrichDocument(
    ...
    const storageKey = buildContentAddressedStorageKey('vlore', 'konsultime', sha256, format.ext);
...
class DrizzleEnrichmentRepository implements EnrichmentRepository {
  async loadKonsultimeRows(): Promise<KonsultimDocumentRow[]> {
    return db
      ...
      .where(and(eq(municipalities.slug, 'vlore'), eq(konsultime.source_origin, SOURCE_ORIGIN)));
  }
  ...
  async linkDocumentAndAudit(input: LinkAuditInput): Promise<boolean> {
    return db.transaction(async (tx) => {
      ...
      await tx.insert(audit_log).values({
        ...
        actor_id: 'scraper:vlore:konsultime-documents',
        payload: {
