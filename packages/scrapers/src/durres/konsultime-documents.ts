...
const DETAIL_URL_PATH_PATTERN = /^\/\d{4}\/\d{2}\/\d{2}\/.+\/$/;
const POST_CONTENT_SELECTOR = '.elementor-widget-theme-post-content';
...
export function isOfficialDurresKonsultimeDetailUrl(value: string): boolean {
  try {
    const url = new URL(value);
    return url.origin === 'https://durres.gov.al' && DETAIL_URL_PATH_PATTERN.test(url.pathname);
  } catch {
    return false;
  }
}

export function extractDurresKonsultimeDocumentLinks(
  html: string,
  detailUrl: string,
): DetailDocumentLink[] {
  ...
  $(`${POST_CONTENT_SELECTOR} a[href*="/wp-content/uploads/"]`).each((_i, el) => {
    ...
    if (url.origin !== 'https://durres.gov.al') return;
    if (!url.pathname.includes('/wp-content/uploads/')) return;
...
export class DurresKonsultimeDocumentEnricher extends BaseScraper {
  ...
  async runWithStats(): Promise<EnrichmentStats> {
    assertDevDatabase(undefined, 'Durrës konsultime document enrichment');
    ...
    this.logger.info(totals, 'Durrës konsultime document enrichment complete');
    return totals;
  }
  ...
  async enrichKonsultim(row: KonsultimDocumentRow): Promise<EnrichmentStats> {
    ...
    if (!isOfficialDurresKonsultimeDetailUrl(row.sourceUrl)) {
      stats.rowsSkipped += 1;
      this.logger.warn(
    ...
    const links = extractDurresKonsultimeDocumentLinks(html, row.sourceUrl);
    if (links.length === 0) {
      this.logger.info(
        { konsultimId: row.id, sourceUrl: row.sourceUrl },
  ...
  async enrichDocument(
    ...
    const storageKey = buildContentAddressedStorageKey('durres', 'konsultime', sha256, format.ext);
...
class DrizzleEnrichmentRepository implements EnrichmentRepository {
  async loadKonsultimeRows(): Promise<KonsultimDocumentRow[]> {
    return db
      ...
      .where(and(eq(municipalities.slug, 'durres'), eq(konsultime.source_origin, SOURCE_ORIGIN)));
  }
  ...
  async linkDocumentAndAudit(input: LinkAuditInput): Promise<boolean> {
    return db.transaction(async (tx) => {
      ...
      await tx.insert(audit_log).values({
        ...
        actor_id: 'scraper:durres:konsultime-documents',
        payload: {
