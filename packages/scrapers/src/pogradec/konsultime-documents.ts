...
export function isOfficialPogradecKonsultimeDetailUrl(value: string): boolean {
  try {
    ...
    return (
      url.origin === 'https://bashkiapogradec.gov.al' && url.href.startsWith(DETAIL_URL_PREFIX)
    );
  } catch {
    return false;
  }
...
export function extractPogradecKonsultimeDocumentLinks(
  html: string,
  detailUrl: string,
): DetailDocumentLink[] {
  ...
  $(`a[href*="${DOCUMENT_PATH_PREFIX}"]`).each((_i, el) => {
    ...
    if (url.origin !== 'https://bashkiapogradec.gov.al') return;
    if (!url.pathname.includes(DOCUMENT_PATH_PREFIX)) return;
    ...
    const label =
      normalizeText(row.find('td').first().text()) ||
...
export class PogradecKonsultimeDocumentEnricher extends BaseScraper {
  ...
  async runWithStats(): Promise<EnrichmentStats> {
    ...
    this.logger.info(totals, 'Pogradec konsultime document enrichment complete');
    return totals;
  }
  ...
  async enrichKonsultim(row: KonsultimDocumentRow): Promise<EnrichmentStats> {
    ...
    if (!isOfficialPogradecKonsultimeDetailUrl(row.sourceUrl)) {
      stats.rowsSkipped += 1;
      this.logger.warn(
    ...
    const links = extractPogradecKonsultimeDocumentLinks(html, row.sourceUrl);
    if (links.length === 0) {
      this.logger.info(
        { konsultimId: row.id, sourceUrl: row.sourceUrl },
...
class DrizzleEnrichmentRepository implements EnrichmentRepository {
  async loadKonsultimeRows(): Promise<KonsultimDocumentRow[]> {
    return db
      ...
      .where(and(eq(municipalities.slug, 'pogradec'), eq(konsultime.source_origin, SOURCE_ORIGIN)));
  }
  ...
  async linkDocumentAndAudit(input: LinkAuditInput): Promise<boolean> {
    return db.transaction(async (tx) => {
      ...
      await tx.insert(audit_log).values({
        ...
        actor_id: 'scraper:pogradec:konsultime-documents',
        payload: {
